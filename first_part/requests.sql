...
-- Запрос на вывод количества заказов, выполненных каждым сотрудником за последний месяц:
SELECT 
  s.staff_id,
  s.staff_name,
  s.staff_surname,
...
--Запрос на вывод списка из десяти самых продаваемых блюд:
SELECT
  d.dish_name,
  d.dish_description,
...
-- Запрос на вывод списка наиболее часто покупаемой категорий блюд и общее количество заказов по этой категории:
SELECT
  d.dish_category,
...
-- Запрос на вывод количетсва клиентов по возрастным группам:
SELECT 
  CASE
...
-- Запрос на вывод срденего веса блюд по категориям:
SELECT 
  d.dish_category,
...
-- Запрос на вывод списка клиентов, заказавших блюда итальянской кухни:
SELECT DISTINCT 
...
-- Запрос на вывод самых дорогих ингредиентов и их поставщиков:
SELECT
    i.ingredient_name,
    i.ingredient_price,
    i.supplier_id,
